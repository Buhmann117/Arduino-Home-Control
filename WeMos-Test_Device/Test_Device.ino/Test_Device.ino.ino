...
bool funkHandler(const HomieRange& range, const String& value) {
  ...
  funkNode.setProperty("dev").setRange(range).send(value);  // Update the state of the led
...
void setup() {
  ...
  Homie_setFirmware("Test Device", "1.0.0");

  Homie.setSetupFunction(setupHandler).setLoopFunction(loopHandler);
  
  funkNode.advertiseRange("dev", 1, NUMBER_OF_LED).settable(funkHandler);
