...
void loopHandler() {
  ...
  if (millis() - lastTemperatureSent >= TEMPERATURE_INTERVAL * 1000UL || lastTemperatureSent == 0) {
    ...
    temperature = temperature-5.3; // Offset 
    humidity = humidity+0.8; // Offset 
...
bool funkGHandler(const HomieRange& range, const String& value) {
  ...
  if (range.index == 1){
    ...
    // Funk Light Chamber
      ...
      if (on = (value == "on")){
        //Homie.getLogger() << "ist 9/on" << endl;
        transmitter.sendDim(4, 15);// last No is repeating the transmitted code 2^(2)=8 times
      }
      else {
        //Homie.getLogger() << "ist 9/off" << endl;
        transmitter.sendDim(4, 0);
      }
  }
  funkGNode.setProperty("dev").setRange(range).send(value);  // Update the state of the led
