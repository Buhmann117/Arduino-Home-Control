...
bool motorHandler(const HomieRange& range, const String& value) {
  ...
  if (range.index == 1){
    if (on = (value == "on")){
      Homie.getLogger() << "go to 45°" << endl;
      servo1.write(45);      // Turn SG90 servo back to 90 degrees (center position)
    }
    else {
      ...
      if (on = (value == "on")){
        Homie.getLogger() << "go to 30°" << endl;
        servo1.write(30);      // Turn SG90 servo back to 90 degrees (center position)
      }
      else {
...
void setup() {
  ...
  Homie_setFirmware("Heater Switches", "1.0.0");
